...
| **第三版（已完成）** | + 权重校准 + 模板系统 + Workbench | Power Mean | 半自动校准 | 支持全面评估 |
| **第四版（已完成）** | Provider 接口 + 导出管道 + 平台化 | Power Mean | 半自动校准 | 跨项目复用 |
| **第五版（已完成）** | + 元素维度 + Buff/DOT + 被动技能 | Power Mean + Buff修正 | 半自动校准 | 多维度精确评估 |
...
- `Skill_cool_time`：冷却时间（毫秒，`IntIntPair[]` 取首条 `.Key`）
  - 注意：`.Key` 是基础冷却值，`.Value` 是属性 ID（用于动态加成查找）
...
| ~~元素维度未纳入 DPS~~ | ✅ v5.0 已实现：元素抗性修正 EHP |
| ~~Buff 控制未计入~~ | ✅ v5.0 已实现：BuffEvaluator（DOT DPS + 控制时长 + 被动 EHP 修正） |
| 权重校准仍为人工 | P2 校准向导未实现 |
| 蒙特卡洛仿真未实现 | P3 延至第三版 |
...
#### ~~方向 A：元素维度纳入~~ ✅ 已在 v5.0 实现

详见第 11 节。

#### ~~方向 B：Buff 维度~~ ✅ 已在 v5.0 实现
...
| 优先级 | 方向 | 工作量 | 状态 |
|---|---|---|---|
| **P0** | C：权重半自动校准 | 3天 | ✅ v3.0 已完成 |
| **P1** | E：关卡维度聚合升级 | 3-4天 | 🔲 待实施 |
| **P1** | A：元素维度 | 1-2天 | ✅ v5.0 已完成 |
| **P2** | B：Buff 维度 | 4-5天 | ✅ v5.0 已完成 |
| **P3** | D：蒙特卡洛仿真 | 1周+ | 🔲 远期目标 |
...
> 目标：将元素维度和 Buff 维度纳入评分管线，实现多维度精确评估。
...
| 数据结构 | 新增字段 | 说明 |
|---|---|---|
| `DatumFoeRow` | `PassiveSkillIds`, `Speed`, `Level`, `ModelId`, `AttackSpeedPro` | 怪物被动技能、速度、等级等 |
| `DatumSkillInfoRow` | `SelfEffectBuffIds` | 技能自带 Buff ID 列表（来自 `Self_effect` 字段）|
| `DatumSkillBlueprint` | `AttachedBuffIds` | 技能附带 Buff（来自 `SkillAddBuffAbilityConfig.BuffIDList`）|
| `DatumHitPoint` | `DamageElement` | 打击点的元素类型（0=物理, 1=冰, 2=火, 3=毒, 4=电）|
| **新增** `DatumBuffConfigRow` | `ConfigId`, `LogicalId`, `AttrValue`, `AttrValue2`, `AttrValue8`, `BaseHitrate`, `LastTime`, `EffectGroup` | Buff 配置（来自 `HRMazeSkilleffectV8Row`）|

#### 新增导出文件

- **`buff_configs.json`**：所有 Buff 配置行，包含 DOT 系数、持续时间、tick 间隔、命中率、效果组等

#### CooldownMs Bug 修复

`Skill_cool_time` 是 `IntIntPair[]` 类型：
- `.Key` = 基础冷却值（毫秒）
- `.Value` = 属性 ID（用于动态加成查找）

之前导出错误地取了 `.Value`（属性 ID），导致冷却时间异常。已修复为取 `.Key`。

### 11.2 BuffEvaluator（新增模块）

新增 `DatumCore/BuffEvaluator/BuffEvaluator.cs`，从怪物的技能链中提取所有关联 Buff，分三类评估：

#### DOT DPS 计算

```
对每个技能 → AttachedBuffIds + SelfEffectBuffIds
  → 查 buff_configs.json 获取 Buff 配置
  → 筛选 AttrValue2 > 0 的 DOT 类 Buff
  → dot_dps_per_buff = (AttrValue2 / 10000) × FinalAtk / max(AttrValue8 / 1000, 0.5)
  → 总 DOT DPS = Σ dot_dps_per_buff
```

#### 控制时长计算

```
筛选 EffectGroup ∈ {眩晕, 冰冻, 石化, 击退, 击飞} 的 Buff
  → control_sec = LastTime / 1000 × BaseHitrate / 10000
  → 总控制时长 = Σ control_sec
```
...
```
遍历怪物的 PassiveSkillIds → 查技能配置 → SelfEffectBuffIds
  → 查 Buff 配置 → 筛选 LastTime ≤ 0 的永久 Buff
  → 存在永久被动 Buff → EHP 修正因子 ×1.3
```
...
```
avgRes = (IceRes + FireRes + PoisonRes + EleRes) / 4
ElementResistanceFactor = 1 + avgRes / 10000

EHP = CalcEHP(HP, Def, baselineAtk) × ElementResistanceFactor × PassiveBuffModifier
```

#### DPS 分解

```
TotalDPS = SkillDPS + DotDPS
```

#### 控制分解

```
TotalControl = SkillControlScore + BuffControlScore
```

### 11.4 EntityScore 分解字段

`EntityScore` 新增以下字段，供前端展示分解详情：

| 字段 | 类型 | 说明 |
|---|---|---|
| `SkillDPS` | float | 技能直接 DPS（原始值）|
| `DotDPS` | float | DOT 持续伤害 DPS |
| `SkillControlScore` | float | 技能控制评分 |
| `BuffControlScore` | float | Buff 控制时长（秒）|
| `ElementResistanceFactor` | float | 元素抗性修正因子（≥1.0）|
| `PassiveBuffModifier` | float | 被动 Buff EHP 修正因子（≥1.0）|

### 11.5 前端可视化增强

详情抽屉（ScoreDashboard 右侧面板）新增：

- **特性标签**：元素抗性标签（cyan）、被动Buff标签（purple）、DOT伤害标签（volcano）、Buff控制标签（green）
...
| 文件 | 变更 |
|---|---|
| `DatumFoeRow.cs`（Unity 侧） | +PassiveSkillIds, +DamageElement, +AttachedBuffIds, +SelfEffectBuffIds, +DatumBuffConfigRow |
| `UnityFoeDataProvider.cs` | 填充新增字段（PassiveSkillIds, SelfEffectBuffIds, DamageElement, AttachedBuffIds） |
| `DatumExportPipeline.cs` | 新增 `ExportBuffConfigs()`；更新 `ExtractBlueprint` 和 `ExportSkillInfo` |
| ⚠ CooldownMs 修复 | `Skill_cool_time[0].Value` → `.Key`（UnityFoeDataProvider + DatumExportPipeline） |

#### DatumPlatform 仓库

| 文件 | 变更 |
|---|---|
| `DatumFoeRow.cs` | 字段与 Unity 导出对齐（IceRes/FireRes/PoisonRes/EleRes），新增 PassiveSkillIds/Speed/Level/ModelId/AttackSpeedPro |
| `IFoeDataProvider.cs` | +GetBuffConfig(int), +GetAllBuffConfigs() |
| `JsonFoeDataProvider.cs` | +buffConfigsJson 参数，加载 Buff 配置 |
| `MonsterSnapshotBuilder.cs` | 简化为 4 种抗性常量 |
| `AttributeResolver.cs` | 简化为直接取基础值，新增元素抗性解析 |
| **新增** `BuffEvaluator.cs` | DOT DPS + 控制时长 + 被动 EHP 修正 |
| `CombatMetrics.cs` | +SkillDPS/DotDPS/SkillControlScore/BuffControlScore/ElementResistanceFactor/PassiveBuffModifier |
| `CombatMetricsCalculator.cs` | 整合元素抗性修正、被动Buff修正、DOT DPS、Buff控制 |
| `EntityScore.cs` | +分解字段（SkillDPS/DotDPS/BuffControl/ElementRes/PassiveBuff） |
| `ScoreAggregator.cs` | 填充分解字段到 EntityScore |
| `ScoreCalculator.cs` | 调用 BuffEvaluator |
| `DatumDataService.cs` | 加载 buff_configs.json |
| `TemplateDiscovery.cs` | 修复 ExtractAttrs 引用（AtkSpeed→AttackSpeedPro 等） |
| `types/datum.ts` | EntityScore +分解字段，DatumFoeRow 更新，+ELEMENT_LABELS/ELEMENT_COLORS |
| `ScoreDashboard/index.tsx` | 详情面板：特性标签 + DPS分解 + EHP修正 + 控制分解 |

---

*最后更新：2026-03-02*  
*版本：v1.0（Editor 工具 MVP），v2.0（技能DPS + 控制双侧 + Power Mean + 玩家基准），v3.0（权重校准 + 模板系统 + Workbench），v4.0（Provider 接口 + 导出管道），v5.0（元素/Buff 维度集成 + 前端可视化增强）*
